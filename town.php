<?php // town.php :: All town functions.
    ...
    function inn()
    { // Resting at the inn restores hp/mp/tp.

        global $userrow, $townrow;

        // Errors.
        if ($userrow["gold"] < $townrow["innprice"]) {
            err("You do not have enough gold to stay at the inn. Please <a href=\"index.php\">go back</a> and try again.");
        }
        if ($userrow["currenthp"] == $userrow["maxhp"] && $userrow["currentmp"] == $userrow["maxmp"] && $userrow["currenttp"] == $userrow["maxtp"]) {
            err("Your HP, MP and TP are already at their maximum levels. You do not need to stay at the Inn tonight.<br /><br /><a href=\"index.php\">Click here</a> to return to the main town screen.");
        }

        if (isset($_POST["submit"])) {

            $users = new Users();
            $users->restAtInn($userrow['id'], $townrow["innprice"]);

            return true;

        } elseif (isset($_POST["abortmission"])) {
            die(header("Location: index.php"));
        }
    ...
    function map()
    { // Buy maps to towns for the Travel To menu.
    ...
    { // Buy items from merchants.
        ...
        */
        ...
        if (isset($_POST["three"])) {
            ...
            // Requirements check.
            if ($newfullitem["requirements"] == false) {
                err("You do not meet one or more of the requirements for this item. Please <a href=\"index.php\">go back</a> and try again.");
            }
            if ($userrow["gold"] < $newfullitem["buycost"]) {
                err("You do not have enough gold in your pocket to buy this item.");
            }
            ...
            for ($i = 1; $i < 7; $i++) {
                ...
                if ($userrow["item" . $i . "idstring"] != "0") {
                    ...
                    if ($ids[0] != 0) {
                        $prefix = $prefixrow[$ids[0]];
                    } else {
                        $prefix = false;
                    }
                    if ($ids[2] != 0) {
                        $suffix = $suffixrow[$ids[2]];
                    } else {
                        $suffix = false;
                    }
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                if (rand(0, 4) == 1) {
                    $prefix = $preprefixrow[rand(0, (sizeof($preprefixrow) - 1))];
                } else {
                    $prefix = false;
                }
                if (rand(0, 4) == 1) {
                    $suffix = $presuffixrow[rand(0, (sizeof($presuffixrow) - 1))];
                } else {
                    $suffix = false;
                }
                $fullitem = builditem($prefix, $baseitem, $suffix, $modrow);
    ...
    function gamble()
    {
        ...
        if (isset($_POST["submit"])) {
            ...
            // Cup errors.
            if (!isset($_POST["cup"])) {
                err("You didn't pick any cup to bet on. Please <a href=\"index.php?do=gamble\">go back</a> and try again.");
            }
            if (!is_numeric($_POST["cup"])) {
                err("You didn't pick any cup to bet on. Please <a href=\"index.php?do=gamble\">go back</a> and try again.");
            }

            // Bet amount errors.
            if (trim($amount) == "") {
                err("Invalid bet amount. Please <a href=\"index.php?do=gamble\">go back</a> and try again.");
            }
            ...
            if ($mode == "hard") {
                ...
                if ($thecup == $thewin) {
                    ...
                    display("Gamble",
                        "You won!<br /><br />You just picked up <b>" . ($amount * 10) . " Gold</b>.<br /><br />Care to <a href=\"index.php?do=gamble&mode=hard\">try again</a> or would you rather go back to <a href=\"index.php\">town</a>?",
                        true, $userrow['id']);
                } else {
                    $userrow["gold"] -= $amount;
                    doquery("UPDATE users SET gold=gold-$amount WHERE id='" . $userrow["id"] . "' LIMIT 1");
                    display("Gamble",
                        "You lost!<br /><br />Sorry buddy, but we're gonna have to take your <b>" . $amount . " Gold</b>.<br /><br />Care to <a href=\"index.php?do=gamble&mode=hard\">try again</a> or would you rather go back to <a href=\"index.php\">town</a>?",
                        true, $userrow['id']);
                }
            ...
            if ($mode == "easy") {
                ...
                if ($thecup == $thewin) {
                    ...
                    display("Gamble",
                        "You won!<br /><br />You just picked up <b>" . ($amount * 2) . " Gold</b>.<br /><br />Care to <a href=\"index.php?do=gamble\">try again</a> or would you rather go back to <a href=\"index.php\">town</a>?");
                } else {
                    $userrow["gold"] -= $amount;
                    doquery("UPDATE users SET gold=gold-$amount WHERE id='" . $userrow["id"] . "' LIMIT 1");
                    display("Gamble",
                        "You lost!<br /><br />Sorry buddy, but we're gonna have to take your <b>" . $amount . " Gold</b>.<br /><br />Care to <a href=\"index.php?do=gamble\">try again</a> or would you rather go back to <a href=\"index.php\">town</a>?");
                }
    ...
    function bank()
    {
        ...
        if (isset($_POST["withdraw"])) {
            ...
            if ($_POST["amount"] > $userrow["bank"]) {
                err("You do not have that much money in the bank.");
            }
            ...
            display("Deposit/Withdraw Gold at the Bank", parsetemplate(gettemplate("town_bank2"), $row), true,
                $userrow['id']);
            ...
            display("Deposit/Withdraw Gold at the Bank", parsetemplate(gettemplate("town_bank1"), $row), true,
                $userrow['id']);
    ...
    function halloffame()
    {
        ...
        $top = dorow(doquery("SELECT *, DATE_FORMAT(birthdate, '%m.%d.%Y') AS fregdate FROM users ORDER BY experience DESC LIMIT 25"),
            "id");
        $row["halltable"] = "";
        $i = 1;
    ...
    function duel()
    {
        ...
        $row = dorow(doquery("SELECT * FROM users WHERE UNIX_TIMESTAMP(onlinetime) >= '" . (time() - 600) . "' AND world='" . $userrow["world"] . "' AND latitude='" . $userrow["latitude"] . "' AND longitude='" . $userrow["longitude"] . "' AND id !='" . $userrow["id"] . "' ORDER BY id"),
            "id");
    ...
    function duelchallenge()
    {
        ...
        if (isset($_GET["uid"])) {
            ...
            if ($_GET["uid"] == $userrow["id"]) {
                err("You cannot duel yourself.");
            }
            ...
            if ($newuserrow["account"] == $userrow["account"]) {
                err("You cannot duel another character on your own account.");
            }
            if ($newuserrow["fonlinetime"] <= (time() - 600)) {
                err("That user is not online.");
            }
            if ($newuserrow["currentaction"] != "In Town") {
                err("That user is busy.");
            }
            if ($newuserrow["latitude"] != $userrow["latitude"] || $newuserrow["longitude"] != $userrow["longitude"]) {
                err("That user is not in this town.");
            }
        } else {
            err("Invalid UID.");
        }
